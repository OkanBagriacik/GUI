...
public class MainActivity extends AppCompatActivity implements NoteFragment.OnNoteListInteractionListener {
    ...
    private static final String TAG = "Firebase Firestore Demo";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!displayingEditor) {
            ...
            ft.add(R.id.noteContainer, NoteFragment.newInstance(), "list_note");
            ft.commit();
        } else {
            FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
            ft.replace(R.id.noteContainer, EditNoteFragment.newInstance(editingNote.getContent()));
        ...
        listenerRegistration = db.collection("notes").orderBy("date",
            ...
            public void onEvent(@Nullable QuerySnapshot queryDocumentSnapshots, @Nullable
                ...
                if (e != null) {
                    Log.e(TAG, "An error occurred while retrieving notes", e);
                    return;
                }
                notes.clear();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.action_new:
                ...
                FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
                ft.replace(R.id.noteContainer, EditNoteFragment.newInstance(""), "edit_note");
                ft.addToBackStack(null);
                ft.commit();
                return true;
    ...
    public void onNoteSelected(Note note) {
        editingNote = note;
        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.noteContainer, EditNoteFragment.newInstance(editingNote.getContent()), "edit_note");
        ft.addToBackStack(null);
        ft.commit();
        displayingEditor = !displayingEditor;
        invalidateOptionsMenu();
